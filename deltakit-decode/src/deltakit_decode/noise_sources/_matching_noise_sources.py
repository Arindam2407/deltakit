...
class NoNoiseMatchingSequence(SequentialNoise[HyperMultiGraph, OrderedDecodingEdges]):
    ...
    def split_error_generator(
        self, code_data: HyperMultiGraph, num_splits: int, seed: Optional[int] = None
    ) -> Tuple[Tuple[Iterator[OrderedDecodingEdges], int], ...]:
        return ((self.error_generator(code_data, seed), 1),) + tuple(
            (_empty_generator(), 0) for _ in range(num_splits - 1)
        )
...
class FixedWeightMatchingNoise(IndependentMatchingNoise):
    ...
    def __init__(self, weight: int, edge_filter: Optional[EdgeFilterT] = None):
        self.weight = weight
    ...
    def __eq__(self, __o: object) -> bool:
        if self.weight == 0 and __o == _ZERO:
            return True
        return (
            isinstance(__o, FixedWeightMatchingNoise)
...
class UniformMatchingNoise(IndependentMatchingNoise):
    ...
    def __init__(
        self, basic_error_prob: float, edge_filter: Optional[EdgeFilterT] = None
    ):
    ...
    def __eq__(self, __o: object) -> bool:
        if self.basic_error_prob == 0 and __o == _ZERO:
            return True
        return (
            isinstance(__o, UniformMatchingNoise)
            and self.basic_error_prob == __o.basic_error_prob
            and self.edge_filter == __o.edge_filter
        )
...
class EdgeProbabilityMatchingNoise(IndependentMatchingNoise):
    ...
    def __add__(self, other):
        raise NotImplementedError(
            "Cannot add edge probability noise to any other noise."
        )
...
class AdditiveMatchingNoise(IndependentMatchingNoise):
    ...
    def __eq__(self, __o: object) -> bool:
        return (
            isinstance(__o, AdditiveMatchingNoise)
            and self._internal_sources_multiset == __o._internal_sources_multiset
        )
...
class ExhaustiveMatchingNoise(SequentialNoise[HyperMultiGraph, OrderedDecodingEdges]):
    ...
    def __init__(
        self, weight: Optional[int], edge_filter: Optional[EdgeFilterT] = None
    ):
        self.weight = weight
    ...
    def __eq__(self, __o: object) -> bool:
        return (
            isinstance(__o, ExhaustiveMatchingNoise)
            and self.weight == __o.weight
            and self.edge_filter == __o.edge_filter
        )
    ...
    def split_error_generator(
        ...
        if self.weight is None:
            raise NotImplementedError(
                "Ascending exhaustive split generator is not implemented."
            )
        if self.weight == 0:
...
class ExhaustiveWeightedMatchingNoise(
    ...
    def __init__(
        self, exhaustion_ceiling: float, edge_filter: Optional[EdgeFilterT] = None
    ):
    ...
    def __eq__(self, __o: object) -> bool:
        return (
            isinstance(__o, ExhaustiveWeightedMatchingNoise)
            and self.exhaustion_ceiling == __o.exhaustion_ceiling
            and self.edge_filter == __o.edge_filter
        )
...
class UniformErasureNoise(
    ...
    def __eq__(self, __o: object) -> bool:
        return (
            isinstance(__o, UniformErasureNoise)
            and self.erasure_probability == __o.erasure_probability
            and self.edge_filter == __o.edge_filter
            and self.pauli_noise_model == __o.pauli_noise_model
        )
    ...
    def __add__(self, other):
        raise NotImplementedError(
            "Cannot add uniform erasure noise to any other noise."
        )
